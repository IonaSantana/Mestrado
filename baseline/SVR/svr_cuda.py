...
from cuml.multioutput import MultiOutputRegressor as cuMultiOutputRegressor
import cupy as cp  # Para trabalhar com GPU
...
def pipeline_all_sensors(file_path, lags_list, results_file, train_ratio=0.6, val_ratio=0.2):
    ...
    for lags in lags_list:
        print(f"\nTestando SVR com {lags} lags... (Usando GPU)")
        try:
            ...
            # Converta os dados para o formato compatível com GPU (cuML usa cupy)
            ...
            # Criar e treinar o modelo MultiOutputRegressor com cuML
            base_model = cuSVR(kernel='rbf', C=1.0, epsilon=0.1)
            model = cuMultiOutputRegressor(base_model)
            model.fit(X_train_gpu, y_train_gpu)

            # Fazer previsões
            predictions_val_gpu = model.predict(X_val_gpu)
            predictions_test_gpu = model.predict(X_test_gpu)

            # Converta as previsões de volta para a CPU
            predictions_val = cp.asnumpy(predictions_val_gpu)
            predictions_test = cp.asnumpy(predictions_test_gpu)
            y_val = cp.asnumpy(y_val_gpu)
...
pipeline_all_sensors(
    ...
    lags_list=[3, 5, 7],
    results_file='sensor_results_SVR_GPU.csv'
)
